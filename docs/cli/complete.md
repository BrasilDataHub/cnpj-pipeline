...
2. **Carrega** os dados no banco PostgreSQL.
...
- **Banco**: PostgreSQL usando as credenciais de `config.py`/variáveis de ambiente.
...
| `--engine`          | `postgres`     | `postgres`              | Tipo do SGBD utilizado.                                                   |
...
| `--db-name`         | `<string>`     | `dados_cnpj`            | Nome do banco de dados PostgreSQL.                                        |
...
| `--parallel`        | _flag_         | _desativado_            | Se usado, utiliza multi thread para a carga de dados                      |

### Exemplo

```bash
python etl.py complete --month 03/2025 --engine postgres --clean
```
