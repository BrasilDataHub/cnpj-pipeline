-- =============================================================================
-- mv_comparativo_cnae - Comparativo entre períodos equivalentes (CNAE)
-- =============================================================================
-- Tempo estimado de criação: ~1 min (lê mv_movimentacao_mensal_cnae)
-- Periodicidade de refresh recomendada: junto de mv_movimentacao_mensal_cnae
-- Depende de: 00_helpers.sql, 14_mv_movimentacao_mensal_cnae.sql
-- =============================================================================
...
-- diferenças:
--   - a entidade é `cod_cnae_principal`, e a chave é (CNAE x nível geográfico);
--   - não existe nível 'municipio': a série de origem para em UF (o produto
--     mês x CNAE x 5.570 municípios não cabe). `cod_cidade_ibge` existe para
--     manter as três MVs de comparativo com o mesmo shape, mas é SEMPRE 0.
--
...
base AS (
    ...
    SELECT
        ...
        m.ainda_ativos,
        m.baixas
    FROM mv_movimentacao_mensal_cnae m
    CROSS JOIN ancora a
...
niveis AS (
    ...
    FROM base b
    GROUP BY b.cod_cnae_principal, b.mes_abertura
    ...
    GROUP BY b.cod_cnae_principal, b.cod_regiao_ibge, b.mes_abertura
    ...
    GROUP BY b.cod_cnae_principal, b.cod_estado_ibge, b.mes_abertura
),
agregados AS (
    SELECT
        n.cod_cnae_principal,
        n.nivel,
        n.cod_regiao_ibge,
        n.cod_estado_ibge,
        j.periodo_meses,
...
CREATE UNIQUE INDEX idx_mv_comp_cnae_pk
    ON mv_comparativo_cnae (cod_cnae_principal, nivel, cod_regiao_ibge, cod_estado_ibge, periodo_meses);
CREATE INDEX idx_mv_comp_cnae_estado
    ON mv_comparativo_cnae (cod_estado_ibge, periodo_meses);
CREATE INDEX idx_mv_comp_cnae_regiao
    ON mv_comparativo_cnae (cod_regiao_ibge, periodo_meses);
CREATE INDEX idx_mv_comp_cnae_variacao
    ON mv_comparativo_cnae (nivel, periodo_meses, aberturas_variacao_pct DESC);

COMMENT ON MATERIALIZED VIEW mv_comparativo_cnae IS
'Comparativo entre períodos equivalentes (1, 6, 12, 24 e 48 meses) por CNAE
principal, nos níveis brasil/região/estado. Mesmo contrato de colunas de
mv_comparativo_territorio.';
COMMENT ON COLUMN mv_comparativo_cnae.nivel IS 'brasil | regiao | estado — define qual coluna de código geográfico está preenchida';
COMMENT ON COLUMN mv_comparativo_cnae.cod_cidade_ibge IS 'Sempre 0: este comparativo não desce a município (existe para uniformizar o shape das MVs de comparativo)';
COMMENT ON COLUMN mv_comparativo_cnae.periodo_meses IS 'Tamanho da janela em meses: 1, 6, 12, 24 ou 48';
COMMENT ON COLUMN mv_comparativo_cnae.aberturas_variacao_pct IS 'NULL quando o período anterior teve zero aberturas (o site mostra "Novo")';
COMMENT ON COLUMN mv_comparativo_cnae.sobrevivencia_delta_pp IS 'Diferença em pontos percentuais (atual - anterior); NULL se algum lado for NULL';

-- Estatísticas para otimização do query planner
ANALYZE mv_comparativo_cnae;
